           ...
           size_t *latlon_count, double *lat, double *lon)
{
    ...
    for (dv = 0; dv < NUM_DATA_VARS; dv++)
    {
        ...
        if (!strcmp(compression_filter_name[f], "szip-NN"))
            if (nc_def_var_szip(ncid, data_varid[dv], NC_SZIP_NN, deflate)) ERR;
        if (!strcmp(compression_filter_name[f], "szip-EC"))
             ...
             int deflate_level[][NUM_DEFLATE_LEVELS])
{
    ...
    /* szip is optionally present. For szip the deflate_level is
     * actual the pixels_per_block parameter. */
#if NC_HAS_SZIP_WRITE
    strcpy(compression_filter_name[nfilters], "szip-NN");
    deflate_level[nfilters][0] = 4;    
    deflate_level[nfilters][1] = 8;    
    deflate_level[nfilters][2] = 16;    
    deflate_level[nfilters][3] = 32;    
    nfilters++;
    strcpy(compression_filter_name[nfilters], "szip-EC");
    deflate_level[nfilters][0] = 4;    
    deflate_level[nfilters][1] = 8;    
    deflate_level[nfilters][2] = 16;    
    deflate_level[nfilters][3] = 32;    
    ...
    /* for (i = 0; i < NUM_DEFLATE_LEVELS; i++) */
    /*     deflate_level[nfilters][i] = -20 + i * 4; */
    deflate_level[nfilters][0] = -4;
    nfilters++;
#endif /* BUILD_ZSTD */
