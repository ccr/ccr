  ! This is a test program for the CCR BitGroom quantization filter for
...
program ftst_ccr_bitgroom
  use netcdf
  use ccr
  implicit none

  ! This is the name of the data file we will create.
  character (len = *), parameter :: FILE_NAME = "ftst_ccr_bitgroom.nc"
  integer :: ncid
  ...
  call check( nf90_def_var_bitgroom(ncid, pres_varid, QUANTIZATION_NSD) )
  call check( nf90_def_var(ncid, TEMP_NAME, NF90_REAL, dimids, temp_varid) )
  call check( nf90_def_var_bitgroom(ncid, temp_varid, QUANTIZATION_NSD) )

  ! Check the quantization settings.
  call check( nf90_inq_var_bitgroom(ncid, pres_varid, bitgroomp, nsdp) )
  if (nsdp .ne. QUANTIZATION_NSD) stop 2
  if (bitgroomp .ne. 1) stop 2
  nsdp = 0
  bitgroomp = 0
  call check( nf90_inq_var_bitgroom(ncid, temp_varid, bitgroomp, nsdp) )
  if (nsdp .ne. QUANTIZATION_NSD) stop 2
  if (bitgroomp .ne. 1) stop 2
